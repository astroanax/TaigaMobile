...
class DataModule {
    ...
    fun provideTaigaApi(session: Session, moshi: Moshi): TaigaApi {
        // Placeholder base URL (must end with a slash for Retrofit)
        ...
        val okHttpBuilder = OkHttpClient.Builder()
            .addInterceptor { chain ->
                ...
                // Build final URL by replacing placeholder with actual API URL
                val finalUrl = request.url.toString().replace(baseUrlPlaceholder, getApiUrl())

                val newRequestBuilder = request.newBuilder()
                    .url(finalUrl)
                    .header("User-Agent", "TaigaMobile/${BuildConfig.VERSION_NAME}")

                // Avoid adding Authorization header for auth endpoints (login/refresh)
                val encodedPath = request.url.encodedPath
                // Auth endpoints may be referenced with or without API prefix, check contains
                val isAuthEndpoint = encodedPath.contains("/${TaigaApi.AUTH_ENDPOINTS}") || encodedPath.contains("/${TaigaApi.REFRESH_ENDPOINT}")

                if (!isAuthEndpoint) {
                    newRequestBuilder.header("Authorization", "Bearer ${session.token.value}")
                }
